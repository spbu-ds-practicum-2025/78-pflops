...
AdService-->|HTTP| UserService

AdService-->|HTTP|MediaService
```
...
-   Интеграция с MiniIO для хранения медиа
-   Реализация API Gateway для маршрутизации и управления запросами
-   Реализация фронтенда для обеспечения всего вышеописанного фунционала
**План разработки:**
...
3.  Реализация системы рейтингов и отзывов (Rating Service)
4.  Добавление функционала продвижения объявлений
5.  Настройка репликации PostgreSQL и балансировки нагрузки
6.  Реализация кэширования (Redis) для повышения производительности
...
-   Система масштабируется горизонтально (одновременная работа 50+ узлов)
-   Система отказоустойчива (продолжает работу при отказе одного из компонентов)
-   80% кода покрыто тестами
-   Обеспечена высокая производительность при нагрузке (время отклика системы на запрос пользователя <300 мс при одновременной работе 100+ человек.)
